...
call s:command("-bar -nargs=? Gstatus :execute s:Status(<q-args>)")
...
if !exists('g:show_untrack_file')
    let g:show_untrack_file = 1
endif

function! s:Status(arg) abort
    try
        if a:arg ==# '-uno'
            let g:show_untrack_file = 0
        else
            let g:show_untrack_file = 1
        endif
        Gpedit :
